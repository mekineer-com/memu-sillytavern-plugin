...
function buildMemuPayloadForLocal(
  ...
  if (llm_profiles["preprocess"]) memorize_config.preprocess_llm_profile = "preprocess";
  if (llm_profiles["memory_extract"]) memorize_config.memory_extract_llm_profile = "memory_extract";
  if (llm_profiles["category_update"]) memorize_config.category_update_llm_profile = "category_update";
...
function getExternalServerBaseUrl(_cfg: MemuPluginConfig): string {
  ...
  if (root) {
    ...
    const listen: any = cfg && typeof cfg.listen === 'object' ? cfg.listen : null;
    if (listen) {
      ...
      if (!hostRaw || port <= 0) {
        throw new Error(`Invalid listen host/port in ${path.join(root, 'config.json')}`);
      }
...
export async function proxyMemorizeConversation(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (rebuild) query.set('rebuild', 'true');
...
export async function proxyConversationRetrieve(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (typeof req.body?.mentalHealthAddon === "boolean") {
      payload.mental_health_addon = req.body.mentalHealthAddon;
    }
