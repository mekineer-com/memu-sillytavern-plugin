...
class MemorizeMixin:
    ...
    async def _preprocess_conversation(
        ...
        Returns a list of resources (one per segment). For short conversations we skip the
        segmentation LLM call and just return a single resource.
        """
        preprocessed_text = format_conversation_for_preprocess(text)
        conversation_text = preprocessed_text  # always use the original JSON-derived text downstream

        # If the conversation is short, don't call the segmenter at all.
        # Each line is one message; memU's prompt guideline is "<20 messages => 1 segment".
        try:
            if preprocessed_text.count("\n") + 1 < 20:
                return [{"text": conversation_text, "caption": None}]
        except Exception:
            # If counting fails for any reason, just proceed with the normal path.
            pass
        ...
        try:
            ...
            logger.exception("Conversation preprocess (segmenter) failed; falling back to a single segment")
            return [{"text": conversation_text, "caption": None}]
